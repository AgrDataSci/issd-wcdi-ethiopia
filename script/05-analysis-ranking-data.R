...
keep = ll_crops$mean_random < ll_crops$share_top

perms = as.data.frame(table(ll_crops$trait[keep], ll_crops$crop[keep]))

write.csv(perms, "output/permutation-result-freq-1-means-share-top-higher-than-random.csv", row.names = FALSE)
